...
def run(runcase, inp, out):
    runcase = runcase
    inp = inp
    out = out
    print("Testing test case %d" % runcase)
    if( runcase == 0 ): # download inference data, trained model
        ...
        dirstr = out # the place to save the results
        testpre = [''] # the test cases
        os.chdir('Enhance')
        ...
        for nn in range(len(testpre)):
            cmd1 = ["python3", "main.py",
                ...
                "--mode","inference",
                "--input_dir_LR", inp,   # the LR directory
                "--output_pre", testpre[nn], # the subfolder to save current scene, optional
                "--num_resblock", "16",  # our model has 16 residual blocks,
                "--checkpoint", 'checkpoints/model-373000', #'ex_TecoGAN10-23-10/model-10000',  # the path of the trained model,
        ...
        #TrainingDataPath = "/content/drive/My Drive/TecoGAN/HR"
        TrainingDataPath = "/cluster/work/infk_mtc/alessiapacca/TecoGAN/HR"
        ...
        train_dir = folder_check("esperimento4_randomcrop_partendodateco%s/"%now_str)
        # train TecoGAN, loss = l2 + VGG54 loss + A spatio-temporal Discriminator
        cmd1 = ["python3", "main.py", "-W ignore"
            "--cudaID", "0", # set the cudaID here to use only one GPU
            "--output_dir", train_dir, # Set the place to save the model.
            ...
            "--max_iter", "1250", # 500k or more, the one we present is trained for 900k
            "--save_freq", "10000", # the frequency we save model
