...
def TecoGAN(r_inputs, r_targets, r_masks, FLAGS, GAN_Flag=True):
    ...
    gen_outputs, gen_warppre, masks_output = [], [], []
    ...
    with tf.variable_scope('get_learning_rate_and_global_step'):
        ...
        learning_rate = tf.train.exponential_decay(FLAGS.learning_rate,
                                                   global_step, FLAGS.decay_step, FLAGS.decay_rate,
    ...
    with tf.variable_scope('generator'):
        ...
        for frame_i in range(inputimages - 1):
            ...
            gen_pre_output_reshape = tf.reshape(gen_pre_output_warp,
            ...
            gen_pre_output_reshape = tf.reshape(gen_pre_output_reshape,
            ...
            '''
            Masks part. I think this would be enough: just appending the frames one after the other? 
            I don't know why it concatenates the input and the gen pre outputs. 
            '''
            masks = r_masks[:, frame_i + 1, :, :, :]
        ...
        mask_output = tf.stack(masks_output, axis = 1)
        ...
        mask_output.set_shape([FLAGS.batch_size, inputimages, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3])
    ...
    s_gen_output = tf.reshape(gen_outputs,
                              (FLAGS.batch_size * inputimages, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3))
    s_masks_output = tf.reshape(mask_output,
                              (FLAGS.batch_size * inputimages, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3))
    ...
    if FLAGS.vgg_scaling > 0.0:
        with tf.name_scope('vgg_19') as scope:
            vgg_layer_labels = ['vgg_19/conv2/conv2_2', 'vgg_19/conv3/conv3_4', 'vgg_19/conv4/conv4_4',
                                'vgg_19/conv5/conv5_4']
            gen_vgg = VGG19_slim(tf.multiply(s_gen_output, s_masks_output), reuse=False, deep_list=vgg_layer_labels)
            target_vgg = VGG19_slim(tf.multiply(s_targets, s_masks_output), reuse=True, deep_list=vgg_layer_labels)

    if (GAN_Flag):  # build the discriminator
        # prepare inputs
        with tf.device('/gpu:0'), tf.name_scope('input_Tdiscriminator'):
            t_size = int(
                3 * (inputimages // 3))  # 3 frames are used as one entry, the last inputimages%3 frames are abandoned
            ...
            if not FLAGS.pingpang:  # backward motion has to be calculated
                fnet_input_back = tf.concat((r_inputs[:, 2:t_size:3, :, :, :], r_inputs[:, 1:t_size:3, :, :, :]),
                                            axis=-1)
                fnet_input_back = tf.reshape(fnet_input_back,
                                             (t_batch, FLAGS.crop_size, FLAGS.crop_size, 2 * output_channel))
                ...
                gen_flow_back = tf.reshape(gen_flow_back,
                                           (FLAGS.batch_size, t_size // 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 2))

                T_inputs_VPre_batch = tf.identity(gen_flow[:, 0:t_size:3, :, :, :])  # forward motion reused,
                T_inputs_V_batch = tf.zeros_like(
                    T_inputs_VPre_batch)  # no motion for middle frames, todo remove for better performance
            ...
            T_vel = tf.stop_gradient(
                T_vel)  # won't passing gradient to fnet from discriminator, details in TecoGAN supplemental paper

        if (
                FLAGS.crop_dt < 1.0):  # crop out unstable part for temporal discriminator, details in TecoGAN supplemental paper
        ...
        with tf.name_scope('real_Tdiscriminator'):
            ...
            with tf.device('/gpu:0'), tf.variable_scope('tdiscriminator', reuse=False):
                ...
                if (FLAGS.crop_dt < 1.0):
                    real_warp = tf.image.crop_to_bounding_box(real_warp, offset_dt, offset_dt, crop_size_dt,
                                                              crop_size_dt)
                ...
                if (FLAGS.Dt_mergeDs):  # a spatio-temporal D
                    ...
                    tdiscrim_real_output = discriminator_F(real_warp,
                                                           FLAGS=FLAGS)  # [tb,h*FLAGS.crop_dt,w*FLAGS.crop_dt,RRRGGGBBB]
        ...
        with tf.name_scope('fake_Tdiscriminator'):
            ...
            with tf.device('/gpu:0'), tf.variable_scope('tdiscriminator', reuse=True):  # reuse weights
                ...
                if (FLAGS.crop_dt < 1.0):
                    fake_warp = tf.image.crop_to_bounding_box(fake_warp, offset_dt, offset_dt, crop_size_dt,
                                                              crop_size_dt)

                if (FLAGS.Dt_mergeDs):  # a spatio-temporal D
                    if (FLAGS.crop_dt < 1.0): fake_warp = tf.pad(fake_warp, paddings, "CONSTANT")
                    with tf.variable_scope('sdiscriminator',
                                           reuse=True):  # actually no more variable under this scope...
                        ...
                        before_warp = tf.reshape(before_warp,
                                                 (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3 * 3))
                        # input_hi is reused
    ...
    with tf.variable_scope('generator_loss'):
        ...
        with tf.device('/gpu:0'), tf.variable_scope('content_loss'):
            ...
            # s_gen_output is a list of tensors, of shape (FLAGS.batch_size*10, 128, 128, 3)
            # ideally we want to detect the face in a single image at the time, but how?
            # do I need to loop it for every single element of batch * 10?
            '''
            Element wise product with the mask list of tensors
            '''
            diff1_mse = tf.multiply((s_gen_output - s_targets), s_masks_output)
        ...
        if FLAGS.vgg_scaling > 0.0:
            with tf.device('/gpu:0'), tf.variable_scope('vgg_layer_loss'):
                ...
                for layer_i in range(vgg_layer_n):
                    ...
                    If I modify them priorly, then this should work. 
                    Check the dimensions though
                    '''
                    #gen_vgg[vgg_layer_labels[layer_i]] = tf.multiply(gen_vgg[vgg_layer_labels[layer_i]] , s_masks_output)
                    #target_vgg[vgg_layer_labels[layer_i]] = tf.multiply(target_vgg[vgg_layer_labels[layer_i]] , s_masks_output)
                    curvgg_diff = tf.reduce_sum(
                        gen_vgg[vgg_layer_labels[layer_i]] * target_vgg[vgg_layer_labels[layer_i]], axis=[3])
