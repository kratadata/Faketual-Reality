...
def loadHRLR(FLAGS, tar_size):
    with tf.device('/cpu:0'):
        ...
        for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
            inputDir = '/cluster/work/infk_mtc/alessiapacca/TecoGAN/HR/0' #os.path.join(FLAGS.input_video_dir, '%s_%04d' % (FLAGS.input_video_pre, dir_i))
            inputDirLR = '/cluster/work/infk_mtc/alessiapacca/TecoGAN/LR/0'
            if (os.path.exists(inputDir)):  # the following names are hard coded
                if not os.path.exists(os.path.join(inputDir, 'res9%04d.png' % FLAGS.max_frm)):
                    print("Skip %s, since folder doesn't contain enough frames!" % inputDir)
                    continue
                for fi in range(FLAGS.RNN_N):
                    image_list_LR_r[fi] += [os.path.join(inputDir, 'res9%04d.png' % frame_i) for frame_i in
                                            range(fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)]
                    image_list_HR_r[fi] += [os.path.join(inputDirLR, 'res9%04d.png' % frame_i) for frame_i in
                                            range(fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)]
        ...
        with tf.variable_scope('load_frame'):
            # define the image list queue
            output = tf.train.slice_input_producer(image_list_LR_r + image_list_HR_r, shuffle=False, capacity=int(FLAGS.name_video_queue_capacity))
            ...
            for fi in range(FLAGS.RNN_N):
                HR_data = tf.image.convert_image_dtype(
                    tf.image.decode_png(tf.read_file(output[fi + FLAGS.RNN_N]), channels=3), dtype=tf.float32)
                ...
                if (FLAGS.random_crop is True) and FLAGS.mode == 'train':
                    ...
                    for frame_t in range(FLAGS.RNN_N):
                        ...
                        target_images[frame_t] = tf.image.crop_to_bounding_box(target_images[frame_t],
                            offset_h, offset_w, tar_size, tar_size)
                        input_images[frame_t] = tf.image.crop_to_bounding_box(input_images[frame_t],
                            offset_h//4, offset_w//4, tar_size//4, tar_size//4)
        ...
        if FLAGS.mode == 'train':
            ...
            raise Exception('Not implemented')
    return batch_list, num_image_list_HR_t_cur
...
def frvsr_data_loader(FLAGS, useValData_ph):  # useValData_ph, tf bool placeholder, whether to use validationdata
    ...
    with tf.name_scope('load_frame_cpu'):
        with tf.name_scope('train_data'):
            print("Preparing train_data")
            batch_list, num_image_list_HR_t_cur = loadHRLR(FLAGS, tar_size)
        with tf.name_scope('validation_data'):
            print("Preparing validation_data")
            val_capacity = 128  # TODO parameter!
            val_q_thread = 1  # TODO parameter!
            valFLAGS = copy_update_configuration(FLAGS,
                                                 {"str_dir": FLAGS.end_dir + 1, "end_dir": FLAGS.end_dir_val,
                                                  ...
                                                  "video_queue_capacity": val_capacity, "queue_thread": val_q_thread})
            vald_batch_list, vald_num_image_list_HR_t_cur = loadHRLR(valFLAGS, tar_size)
    ...
    LR_images_vald = list(vald_batch_list[FLAGS.RNN_N:2*FLAGS.RNN_N:])#
    ...
    # for Ds, inputs_batch and targets_batch are just the input and output:
    S_inputs_frames = tf.stack(input_images, axis=1)  # batch, frame, FLAGS.crop_size, FLAGS.crop_size, 3
    S_targets_frames = tf.stack(target_images, axis=1)  # batch, frame, FLAGS.crop_size*4, FLAGS.crop_size*4, 3
    S_inputs_frames.set_shape((FLAGS.batch_size, FLAGS.RNN_N, FLAGS.crop_size, FLAGS.crop_size, 3))
    S_targets_frames.set_shape((FLAGS.batch_size, FLAGS.RNN_N, 4 * FLAGS.crop_size, 4 * FLAGS.crop_size, 3))
    ...
    useData = tf.cond(useValData_ph, getValdHRpath, getTrainHRpath)
