...
def loadHR_batch(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope('load_frame'):
            for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
                ...
                if (os.path.exists(inputDir)):  # the following names are hard coded: col_high_
                    if not os.path.exists(os.path.join(inputDir, 'res1%04d.png' % FLAGS.max_frm)):
                        print("Skip %s, since foler doesn't contain enough frames!" % inputDir)
                        continue

                    image_list = [os.path.join(inputDir, 'res1%04d.png' % frame_i)
        ...
        with tf.name_scope('sequence_data_preprocessing'):
            ...
            if FLAGS.random_crop and FLAGS.mode == 'train':
                ...
                offset_w = tf.cast(tf.floor(tf.random_uniform([], 0,
                                                              tf.cast(HR_shape[-2], tf.float32) - tar_size)),
                                   dtype=tf.int32)
                offset_h = tf.cast(tf.floor(tf.random_uniform([], 0,
                                                              tf.cast(HR_shape[-3], tf.float32) - tar_size)),
                                   dtype=tf.int32)
            else:
                ...
                if FLAGS.random_crop and FLAGS.mode == 'train':
                    ...
                    if FLAGS.movingFirstFrame:
                        ...
                        HR_sequence_crop_1 = tf.image.crop_to_bounding_box(HR_data_0,
                                                                           ...
                                                                           fix_off_w + lefttop_pos[fi][0], tar_size,
                                                                           tar_size)
        ...
        if len(target_images) != FLAGS.RNN_N:
            raise ValueError('Length of target image sequence is incorrect,expected {}, got {}.'.format(FLAGS.RNN_N,
                                                                                                        len(
                                                                                                            target_images)))
...
def tensor_change_size(input, dim):
    img = Image.open(input)  # this should be an object image
    newsize = (dim, dim)
    im1 = img.resize(newsize, 1)
    tensor = tf.image.convert_image_dtype(im1, dtype=tf.float32)
    return tensor

# load hi-res, lowr-res data from disk
def loadHRLR(FLAGS):
    with tf.device('/cpu:0'):
        # Check the input directory
        if (FLAGS.input_video_dir == ''):
        ...
        image_list_MASKS_r = [[] for _ in range(FLAGS.RNN_N)]  # all empty lists

        for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
            inputDir = '/cluster/work/infk_mtc/alessiapacca/TecoGAN/HR/' + str(dir_i)
            inputDirLR = '/cluster/work/infk_mtc/alessiapacca/TecoGAN/LR/' + str(dir_i)
            inputDirMASKS = '/cluster/work/infk_mtc/alessiapacca/TecoGAN/MaskedDataset/HR/' + str(dir_i)
            ...
            if (os.path.exists(inputDir)):  # the following names are hard coded
                if not os.path.exists(os.path.join(inputDir, 'res1%04d.png' % FLAGS.max_frm)):
                    ...
                    continue
                for fi in range(FLAGS.RNN_N):
                    image_list_LR_r[fi] += [os.path.join(inputDirLR, 'res1%04d.png' % frame_i) for frame_i in
                                            range(fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)]
                    image_list_HR_r[fi] += [os.path.join(inputDir, 'res1%04d.png' % frame_i) for frame_i in
                                            range(fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)]
                    image_list_MASKS_r[fi] += [os.path.join(inputDirMASKS, 'res1%04d.png' % frame_i) for frame_i in
                                            range(fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)]
        ...
        image_list_MASKS_r = [tf.convert_to_tensor(_, dtype=tf.string) for _ in image_list_MASKS_r]

        with tf.variable_scope('load_frame'):
            # define the image list queue
            output = tf.train.slice_input_producer(image_list_LR_r + image_list_HR_r + image_list_MASKS_r, shuffle=False,
                                                   capacity=int(FLAGS.name_video_queue_capacity))
            output_names = output[FLAGS.RNN_N:2*FLAGS.RNN_N:]  # HR names, ignore LR names

            data_list_LR_r = []  # low res rgb, in range 0-1, shape any
            data_list_HR_r = []  # high res rgb, in range 0-1, shape any
            data_list_MASK_r = [] #Masks for faces.
            ...
            for fi in range(FLAGS.RNN_N):
                ...
                with sess.as_default():
                    ...
                    HR_data = tf.image.convert_image_dtype((tf.image.decode_png(tf.read_file(output[fi + FLAGS.RNN_N]), channels=3)), dtype=tf.float32)
                    MASK_data = tf.image.convert_image_dtype((tf.image.decode_png(tf.read_file(output[fi + 2 * FLAGS.RNN_N]), channels=3)), dtype=tf.uint8)
                    MASK_data = 1 - MASK_data
                    MASK_data = tf.cast(MASK_data, dtype=tf.float32)
                ...
                data_list_MASK_r.append(MASK_data)

            input_images = data_list_LR_r
            target_images = data_list_HR_r
            masks_images = data_list_MASK_r
        ...
        with tf.name_scope('data_preprocessing'):
            ...
            with tf.name_scope('random_crop'):
                ...
                if (FLAGS.random_crop is True) and FLAGS.mode == 'train':
                    print('[Config] Use random crop')
                    # Set the shape of the input image to crop_size directly. the target will have 4X size, tar_size is not needed
                    ...
                    for frame_t in range(FLAGS.RNN_N):
                        input_images[frame_t] = tf.image.crop_to_bounding_box(input_images[frame_t],
                                                                              offset_h, offset_w, FLAGS.crop_size,
                                                                              FLAGS.crop_size)
                        target_images[frame_t] = tf.image.crop_to_bounding_box(target_images[frame_t],
                                                                               offset_h * 4, offset_w * 4,
                                                                               FLAGS.crop_size * 4, FLAGS.crop_size * 4)
                        masks_images[frame_t] = tf.image.crop_to_bounding_box(masks_images[frame_t],
                                                                               offset_h * 4, offset_w * 4,
                                                                               FLAGS.crop_size * 4, FLAGS.crop_size * 4)
            ...
            with tf.variable_scope('random_flip'):
                ...
                if (FLAGS.flip is True) and (FLAGS.mode == 'train'):
                    ...
                    for frame_t in range(FLAGS.RNN_N):
                        ...
                        masks_images[frame_t] = random_flip(masks_images[frame_t], flip_decision)

            for frame_t in range(FLAGS.RNN_N):
                input_images[frame_t].set_shape([FLAGS.crop_size, FLAGS.crop_size, 3])
                target_images[frame_t].set_shape([FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3])
                masks_images[frame_t].set_shape([FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3])

        if FLAGS.mode == 'train':
            print('Sequenced batches: {}, sequence length: {}'.format(num_image_list_HR_t_cur, FLAGS.RNN_N))
            batch_list = tf.train.shuffle_batch(output_names + input_images + target_images + masks_images,
                                                batch_size=int(FLAGS.batch_size),
        ...
        paths_HR_batch = batch_list[:FLAGS.RNN_N]
        inputs_batch = list(batch_list[FLAGS.RNN_N:2 * FLAGS.RNN_N:])
        targets_batch = list(batch_list[2 * FLAGS.RNN_N:3*FLAGS.RNN_N:])
        masks_batch = list(batch_list[3 * FLAGS.RNN_N::])

        for frame_t in range(FLAGS.RNN_N):
            inputs_batch[frame_t] = preprocessLR(inputs_batch[frame_t])
            targets_batch[frame_t] = preprocess(targets_batch[frame_t])
            inputs_batch[frame_t].set_shape([FLAGS.batch_size, FLAGS.crop_size, FLAGS.crop_size, 3])
            targets_batch[frame_t].set_shape([FLAGS.batch_size, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3])

            masks_batch[frame_t] = preprocess(masks_batch[frame_t])
            masks_batch[frame_t].set_shape([FLAGS.batch_size, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3])

        # for Ds, inputs_batch and targets_batch are just the input and output:
        S_inputs_frames = tf.stack(inputs_batch, axis=1)  # batch, frame, FLAGS.crop_size, FLAGS.crop_size, 3
        S_targets_frames = tf.stack(targets_batch, axis=1)  # batch, frame, FLAGS.crop_size*4, FLAGS.crop_size*4, 3
        S_masks_frames = tf.stack(targets_batch, axis=1)  # batch, frame, FLAGS.crop_size*4, FLAGS.crop_size*4, 3
        ...
        S_masks_frames.set_shape((FLAGS.batch_size, FLAGS.RNN_N, 4 * FLAGS.crop_size, 4 * FLAGS.crop_size, 3))

    # Define the returned data batches
    Data = collections.namedtuple('Data', 'paths_HR, s_inputs, s_targets, s_masks, image_count, steps_per_epoch')
    return Data(
        paths_HR=paths_HR_batch,
        s_inputs=S_inputs_frames,  # batch, frame, FLAGS.crop_size, FLAGS.crop_size, 3
        s_targets=S_targets_frames,  # batch, frame, FLAGS.crop_size*4, FLAGS.crop_size*4, 3
        s_masks=S_masks_frames,
        image_count=num_image_list_HR_t_cur,
        steps_per_epoch=steps_per_epoch
    )


def frvsr_data_loader(FLAGS, useValData_ph):  # useValData_ph, tf bool placeholder, whether to use validationdata

    with tf.name_scope('load_frame_cpu'):
        with tf.name_scope('train_data'):
            print("Preparing train_data")
            trainData = loadHRLR(FLAGS)
    ...
    useData = getTrainData()
    if useValData_ph == True:
        useData = getValdData()
    else:
        useData = getTrainData()
    #useData = tf.cond(useValData_ph, getValdData, getTrainData)
