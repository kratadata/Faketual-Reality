...
def loadHRLR(FLAGS):
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope('load_frame'):
            ...
            output = tf.train.slice_input_producer(image_list_LR_r + image_list_HR_r, shuffle=False,
                                                   capacity=int(FLAGS.name_video_queue_capacity))
            output_names = output[FLAGS.RNN_N::]  # HR names, ignore LR names
            ...
            for fi in range(FLAGS.RNN_N):
                ...
                with sess.as_default():
                    ...
                    #HR_data = tensor_change_size((output[fi + FLAGS.RNN_N]).eval(), FLAGS.crop_size*4)
                    ...
                    #LR_data = tensor_change_size((output[fi]).eval(), FLAGS.crop_size)
                #tf.image.convert_image_dtype(tf.image.resize_images(tf.image.decode_png(tf.read_file(output[fi]), channels=3), [FLAGS.crop_size, FLAGS.crop_size]), dtype=tf.float32)
                #HR_data = tf.image.convert_image_dtype((tf.image.decode_png(tf.read_file(output[fi + FLAGS.RNN_N]), channels=3)), dtype=tf.float32)
                #LR_data = tf.image.convert_image_dtype((tf.image.decode_png(tf.read_file(output[fi]), channels=3)), dtype=tf.float32)
        ...
        targets_batch = list(batch_list[2 * FLAGS.RNN_N::])
