...
def compute_bbox(start, end, fps, tube_bbox, frame_shape, inp, image_shape, increase_area=0.1):
    ...
    #return f'ffmpeg -i {inp} -c copy -bsf:v h264_metadata=crop_left={left}:crop_right={w-left}:crop_top={top}:crop_bottom={h-top} output.mp4'
    #return f'ffmpeg -i {inp} -vf "crop={w}:{h}:{left}:{top}" -c:v libx264 -crf 0 -c:a copy output.mp4'
    name = os.path.splitext(inp)[0]
    head_tail = os.path.split(name)

    # print head and tail
    # of the specified path
    #head will be head_tail[0]
    #tail will be head_tail[1]

    return f'ffmpeg -i {inp} -vf "crop={w}:{h}:{left}:{top}, scale={scale}" -c:v ffv1 -c:a copy {head_tail[0]}/cropped/{head_tail[1]}output.mkv'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inp", required=False, help='Input image or video')
    parser.add_argument("--min_frames", type=int, default=150,  help='Minimum number of frames')
    parser.add_argument("--cpu", dest="cpu", action="store_true", help="cpu mode.")
    parser.add_argument("--inpdir", required=True, help='Input dir')

    args = parser.parse_args()

    for filename in os.listdir(args.inpdir):
        if (filename.endswith(".mp4")):  # or .avi, .mpeg, whatever.
            args.inp = os.path.join(args.inpdir, filename)
            print(args.inp)
